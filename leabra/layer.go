...
type Layer struct {
	...
	BaseGi  float64         `desc: "same as starting Inhib.Layer.Gi value. Needed to add this for oscillation calculation, since Gi changes with each cycle."`
...
func (ly *Layer) AllParams() string {
	...
	str += "Learn: {\n " + JsonToParams(b)
	str += fmt.Sprintf("OscAmnt: %f \n", ly.OscAmnt) //+ strconv.ParseFloat(, 64) + ""
	for _, pj := range ly.RcvPrjns {
		pstr := pj.AllParams()
		str += pstr
	}
...
func (ly *Layer) WriteWtsJSON(w io.Writer, depth int) {
	...
	w.Write(indent.TabBytes(depth))
	neurons := make([]Neuron, 0, len(ly.Neurons))
	// fmt.Printf("Neurons", len(ly.Neurons), "\n")
	for _, neuron := range ly.Neurons {
		if !neuron.IsOff() {
	...
	nn := len(neurons)
	if nn == 0 {
		w.Write([]byte(fmt.Sprintf("\"Neurons\": null\n")))
	} else {
		w.Write([]byte(fmt.Sprintf("\"Neurons\": {\n")))
		depth++
		// Write the Activations
		w.Write(indent.TabBytes(depth))
		w.Write([]byte(fmt.Sprintf("\"Activations\": [")))
		for ni, neuron := range neurons {
			w.Write([]byte(fmt.Sprintf("%v", neuron.Act)))
			// pj.WriteWtsJSON(w, depth) // this leaves prjn unterminated
			if ni == nn-1 {
				w.Write([]byte(" "))
			} else {
...
func (ly *Layer) SeparateClusters(start int, array []float64) int {
	start_ind := len(array)-start
	if (array[start_ind] < 0.05) || (array[start_ind-1] < 0.01) {
		return start_ind
	}

	// diff_to_compare := array[start_ind] - array[start_ind - 1]
	target_diff := ly.Inhib.Target_diff

	// Default end == -1 means that K_max = infinity
	var end_ind int
	end_ind = 0 
	 
	// fmt.Println("array", array)
	var i int
	for i = len(array) - start - 1; end_ind <= i; i-- {
		// Since the array is sorted, encountering a zero means that
		// all elements to the left is also zero
		if array[i] < 0.01 {
			return i + 1
		}
		current_diff := array[start_ind] - array[i]
		
		// diff_ratio := current_diff - diff_to_compare

		// fmt.Println("diff_to_compare", diff_to_compare, "array[start_ind] - array[i]", array[start_ind], array[i], "current_diff", current_diff, "diff_ratio", diff_ratio)
...
func (ly *Layer) InhibFmGeAct(ltime *Time) {
	...
	} else if ly.Inhib.InhibType == "KWTA" {
		k_max_gi_arr := make([]float64, 50)
		for ni := range ly.Neurons {
			nrn := &ly.Neurons[ni]
			var gi float64

			if nrn.VmOverThr == 0 {
				gi = float64(ly.Act.GiforVmThrfmGe(nrn))
			} else {
				gi = float64(ly.Act.GiforGeThrfmGe(nrn))
			}
			k_max_gi_arr[ni] = math.Max(0, gi)
		}
		
		sort.Float64s(k_max_gi_arr)
		var cap_KWTA_GI float64
		cluster_separator := ly.SeparateClusters(ly.Inhib.K_for_WTA, k_max_gi_arr)
		

		if cluster_separator == -1 {
			panic("Something went wrong with cluster_separator algorithm")
		}
		KWTA_flex_GI := float64(((1 - ly.Inhib.K_point))* (k_max_gi_arr[cluster_separator ]) + ly.Inhib.K_point * (k_max_gi_arr[cluster_separator - 1]))
		KWTA_flex_GI *= float64(ly.Inhib.Layer.Gi) // this is to incorporate the oscillations. The Gi SHOULD default to 1, so this only has an effect once oscillations start.
		// Set cap GI value, subject to oscillation
		if ly.Inhib.K_max != -1 {
			cap_KWTA_GI = k_max_gi_arr[ly.Inhib.K_max ] //* float64( ly.Inhib.Layer.Gi)
		} else {
			cap_KWTA_GI = float64(math.Inf(-1)) // otherwise set cap_KWTA_GI at negative infinity
		}
		
		
		newgivalue := float32(math.Max(KWTA_flex_GI, cap_KWTA_GI))
...
func (ly *Layer) QuarterFinal(ltime *Time) {
	...
	for ni := range ly.Neurons {
		...
		case 2:
			...
			if nrn.HasFlag(NeurHasTarg) && (ly.Learn.LearningMP == 1)  { // will be clamped in plus phase
				nrn.Ext = nrn.Targ
