...
func (ac *ActParams) GiforVmThrfmGe(nrn *Neuron) float32 {
	...
	gk := nrn.Gk * ac.Gbar.K
	vm := nrn.Vm
	githr := ac.XX1.Thr - vm - ac.Dt.VmDt * ge*(ac.Erev.E-vm)
	githr -= ac.Dt.VmDt * ac.Gbar.L*(ac.Erev.L-vm)
	githr -= ac.Dt.VmDt * gk*(ac.Erev.K-vm)
	githr /= (ac.Erev.I-vm) * ac.Dt.VmDt
	githr /= ac.Gbar.I

	nrn.GiThr = githr
	// newvm := nrn.Vm + ac.Dt.VmDt*ac.InetFmG(nrn.Vm, ge, githr, gk)
...
func (ac *ActParams) GiforGeThrfmGe(nrn *Neuron) float32 {
	ge := nrn.Ge * ac.Gbar.E //+ 0.01
	getarget := ge * ac.ThrSubErev.E
	getarget -= ac.Gbar.K*nrn.Gk*ac.ErevSubThr.K
	getarget -= ac.Gbar.L*ac.ErevSubThr.L
	getarget /= ac.Gbar.I * ac.ErevSubThr.I

	nrn.GiThr = getarget
	// invert := ((ac.Gbar.I* getarget *ac.ErevSubThr.I + ac.Gbar.L*ac.ErevSubThr.L + ac.Gbar.K*nrn.Gk*ac.ErevSubThr.K) / ac.ThrSubErev.E)
	// fmt.Println("actual ge", nrn.Ge * ac.Gbar.E, "ge", ge, "invert", invert)
	// newvm := nrn.Vm + ac.Dt.VmDt*ac.InetFmG(nrn.Vm, ge, githr, gk)
	// fmt.Println("newvm", newvm)
	return getarget
}
...
func (ac *ActParams) ActFmG(nrn *Neuron) {
	...
	if nrn.Act < ac.XX1.VmActThr && nrn.Vm <= ac.XX1.Thr {
		...
		nrn.GeThr = geThr
		nrn.GeThr_diff = ge - geThr
		nwAct = ac.XX1.NoisyXX1(nrn.GeThr_diff)
...
type WtInitParams struct {
	...
	// Schlichting
	BlockedInterleaveCondition string `desc:"Modeling Schlichting experiment: whether to use blocked or interleave condition in hidden to output connection when initializing`
...
func (wp *WtInitParams) Defaults() {
	...
	wp.Sym = true
	wp.InitStrategy = "Rand"
	wp.NumOverlapUnits = 2
	wp.NumTotalUnits = 6
}
