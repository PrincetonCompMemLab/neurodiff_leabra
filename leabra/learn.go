...
func (ls *LearnSynParams) CHLdWt(suAvgSLrn, suAvgM, ruAvgSLrn, ruAvgM, ruAvgL, LTD_mult float32) (err, bcm float32) {
	...
	srm := suAvgM * ruAvgM
	if (ls.NMPH == true) {
		bcm = ls.XCal.NMPH(srs)
	} else {
		bcm = ls.XCal.DWt(srs, ruAvgL, LTD_mult)
	}
	err = ls.XCal.DWt(srs, srm, LTD_mult)
	return
}
...
func (ls *LearnSynParams) BCMdWt(suAvgSLrn, ruAvgSLrn, ruAvgL, LTD_mult float32) float32 {
	srs := suAvgSLrn * ruAvgSLrn
	return ls.XCal.DWt(srs, ruAvgL, LTD_mult)
}
...
type XCalParams struct {
	...
	DRev_NMPH    float32 `def:"0.1" min:"0" max:"0.99" desc:"LTD value above which the slope of the weight change is positive and below which the slope of the weight change is negative"`
	...
	DRevMag_NMPH float32 `def:"0.1" min:"0" max:"0.99" desc:"y-value (magnitude) of the lowest point in LTD range where the DWt function reaches its minimum value"`
...
func (xc *XCalParams) Update() {
	...
	if xc.DThr_NMPH >= xc.DRev_NMPH {
		panic("xc.DThr_NMPH cannot be set larger than xc.DRev_NMPH")
	}
	if xc.DRev_NMPH >= xc.ThrP_NMPH {
		panic("xc.DRev_NMPH cannot be set larger than xc.ThrP_NMPH")
	}
	if xc.DRevMag_NMPH > 0 {
		panic("xc.DRevMag_NMPH must be negative")
	}
	if xc.DMaxMag_NMPH < 0 {
		panic("xc.DMaxMag_NMPH must be positive")
	}

	xc.SecondSegSlope_NMPH = (xc.DRevMag_NMPH) / (xc.DRev_NMPH - xc.DThr_NMPH)
	xc.ThirdSegSlope_NMPH  = (xc.DRevMag_NMPH) / (xc.DRev_NMPH - xc.ThrP_NMPH)
	xc.FourthSegSlope_NMPH  = (xc.DMaxMag_NMPH) / (1 - xc.ThrP_NMPH)
...
func (xc *XCalParams) Defaults() {
	...
	xc.SetLLrn = false
	xc.LLrn = 1
	// XCAL function params
	xc.DRev = 0.1
	xc.DThr = 0.0001
	xc.LrnThr = 0.01
	xc.LTD_mult = 1

	// NMPH function params
	xc.DThr_NMPH = 0.0001
	xc.DRev_NMPH = 0.1
	xc.DRevMag_NMPH = -0.5
	xc.DMaxMag_NMPH = .5
...
func (xc *XCalParams) NMPH(srval float32) float32 {
	var dwt float32
	if srval < xc.DThr_NMPH {
		dwt = 0.0
	} else if srval < xc.DRev_NMPH {
		dwt = xc.SecondSegSlope_NMPH * (srval - xc.DThr_NMPH)
	} else if srval < xc.ThrP_NMPH {
		dwt = xc.ThirdSegSlope_NMPH * (srval - xc.ThrP_NMPH)
	} else {
		dwt = xc.FourthSegSlope_NMPH * (srval - xc.ThrP_NMPH)
	}
